...
[GlobalClass, GodotClassName("InputEventHandler"), Tool]
public partial class InputEventHandler : Node
{
	...
	[Export]
	public bool UseInputAcumulation = false;

	Dictionary<Enum, List<(InputAction, int)>> _inputs = new();

	public Dictionary<InputAction, EventActuator> Events { get; private set;} = new();

	public delegate void CurrentChangedDelegate();

	public static CurrentChangedDelegate CurrentChanged;
	...
	public override void _Ready()
	{
		...
		if (!Engine.IsEditorHint())
		{
			...
			Input.SetUseAccumulatedInput(UseInputAcumulation);
		}
	}
